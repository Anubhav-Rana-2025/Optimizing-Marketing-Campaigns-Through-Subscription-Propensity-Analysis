...
def hello_gcs(cloud_event):
    ...
    load_web_analytics_data_to_BQ(bucket,name)

def load_web_analytics_data_to_BQ (bucket_name,filename) :
    gcs_client_obj = storage.Client()
    bq_client_obj = bigquery.Client()
    table_id = 'customer-marketing-analytics.customer_and_marketing_data.web_analytics_data'
    #temp file path to store the uploaded file
    current_year = dt.datetime.now().year
    pattern =rf'^web_analytics_tool_data_{current_year}'
    if (re.match(pattern,filename)):
        ...
        try :
            table =bq_client_obj.get_table('customer-marketing-analytics.customer_and_marketing_data.web_analytics_data')
